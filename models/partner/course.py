...
class Class(Base):
    ...
    course_id = Column(
        BigInteger,
        ForeignKey("partner.courses.id", ondelete="CASCADE"),
        nullable=False,
    )

    name = Column(Text, nullable=False)  # section name
    section_code = Column(Text, nullable=True)  # unique per course (NULLS NOT DISTINCT)
    ...
    __table_args__ = (
        UniqueConstraint(
            "course_id",
            "section_code",
            name="uq_classes_course_section",
...
class InviteCode(Base):
    ...
    created_by_user = relationship("PartnerUser", passive_deletes=True)

    __table_args__ = (
        UniqueConstraint("code", name="uq_invite_codes_code"),
        CheckConstraint("uses_count >= 0", name="chk_invite_codes_uses_nonnegative"),
        CheckConstraint(
            "(max_uses IS NULL) OR (uses_count <= max_uses)",
            name="chk_invite_codes_uses_le_max",
        ),
        Index("idx_invite_codes_class_active", "class_id", "is_active"),
        {"schema": "partner"},
    )
